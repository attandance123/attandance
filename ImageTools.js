...
export default class ImageTools {
    ...
    resize = (file, maxDimensions) => new Promise((resolve) => {
        console.log("called")
        if (!this.browserSupport || !file.type.match(/image.*/)) return resolve(file,false);  // early exit - not supported

        if (file.type.match(/image\/gif/)) return resolve(file,false); // early exit - could be an animated gif
        ...
        image.onload = () => {
            ...
            } else return resolve(file,false); // early exit; no need to resize
            ...
            EXIF.getData(image, () => {
                ...
                else resolve(this.toBlob(imageCanvas, file.type),true);
            });
        };
    ...
    crop = (file, dimensions) => new Promise((resolve) => {
        ...
        image.onload = () => {
            ...
            if (dimensions.width > image.width && dimensions.height > image.height) return resolve(file,false); // early exit - no need to resize
